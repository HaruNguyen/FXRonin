...
function generateTradeCardImage(setup, currentPrice) {
  return new Promise((resolve) => {
    ...
    const draw = (bgImg) => {
      ...
      // ── DARK GRADIENT OVERLAY — transparent top → near-solid dark bottom ──
      const fadeGrd = ctx.createLinearGradient(0, H * 0.32, 0, H * 0.72);
      fadeGrd.addColorStop(0,    "transparent");
      fadeGrd.addColorStop(0.45, "rgba(6,8,8,0.72)");
      fadeGrd.addColorStop(0.80, "rgba(6,8,8,0.94)");
      fadeGrd.addColorStop(1,    "rgba(6,8,8,0.97)");
      ctx.fillStyle = fadeGrd; ctx.fillRect(0, H * 0.32, W, H * 0.68);
      ...
      // ═══════════════════════════════════════════
      const cY0    = Math.round(H * 0.585);  // ~786px — content starts

      // Token name
      ctx.fillStyle = "#FFFFFF"; ctx.font = "bold 54px Inter, Arial"; ctx.textAlign = "left";
      ...
      const pnlY    = sideY + 118;
      const usdtStr = usdtPnl !== null ? `${pnlSign}${fmtC(usdtPnl, 2)}` : `${isLong ? "+" : "−"}0,00`;
      ctx.fillStyle = pnlColor; ctx.font = "bold 100px Inter, Arial"; ctx.textAlign = "left";
      ctx.fillText(usdtStr, PAD, pnlY);
      const pnlTW = ctx.measureText(usdtStr).width;
      ctx.fillStyle = "rgba(255,255,255,0.60)"; ctx.font = "bold 34px Inter, Arial";
      ctx.fillText(" USDT", PAD + pnlTW, pnlY - 20);

      // ── PnL % ──
      const pctStr = pnlPct !== null ? `${pnlSign}${fmtC(pnlPct, 2)}%` : "Pending sync";
      ctx.fillStyle = pnlColor; ctx.font = "bold 52px Inter, Arial"; ctx.textAlign = "left";
      ctx.fillText(pctStr, PAD, pnlY + 68);

      // ── THIN SEPARATOR 1 ──
      const sep1Y = pnlY + 110;
      ctx.strokeStyle = "rgba(255,255,255,0.14)"; ctx.lineWidth = 1;
      ctx.beginPath(); ctx.moveTo(PAD, sep1Y); ctx.lineTo(W - PAD, sep1Y); ctx.stroke();

      // ── ENTRY / LAST PRICE ──
      const priceLabY = sep1Y + 38;
      const priceValY = priceLabY + 50;
      const col2X     = Math.round(W * 0.50);
      ctx.fillStyle = "rgba(255,255,255,0.50)"; ctx.font = "22px Inter, Arial"; ctx.textAlign = "left";
      ...
      // ── Binance diamond logo (canvas-drawn, larger) ──
      const logoX = PAD, logoY = sep2Y + 16;
      const ds    = 14;   // diamond half-size (bigger)
      const drawD = (cx, cy) => {
        ctx.save(); ctx.translate(cx, cy); ctx.rotate(Math.PI/4);
        ctx.fillRect(-ds/2, -ds/2, ds, ds); ctx.restore();
      };
      ctx.fillStyle = "#F0B90B";
      const dg = ds + 4; // spacing between diamond centers
      ...
      // "BINANCE" + "FUTURES" + referral stacked vertically
      const txtX = logoX + logoW + 14;
      ctx.fillStyle = "#F0B90B"; ctx.font = "bold 28px Inter, Arial"; ctx.textAlign = "left";
      ctx.fillText("BINANCE", txtX, logoY + 24);
      ctx.fillStyle = "#FFFFFF"; ctx.font = "bold 28px Inter, Arial";
      ctx.fillText("FUTURES", txtX, logoY + 56);
      // Referral code directly below FUTURES
      ctx.fillStyle = "rgba(255,255,255,0.55)"; ctx.font = "16px Inter, Arial";
      ctx.fillText("Referral Code HARUNGUYEN", PAD, logoY + 86);
